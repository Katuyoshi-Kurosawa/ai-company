...
/**
 * AgentSidePanel -- 360px固定幅の右サイドパネル
 * AgentDetailModal と同じ6タブ構成をインラインパネルとして表示する
 */
      ...
      <div className="flex items-center gap-3 p-4 border-b border-[#262a38]">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs opacity-50 truncate">{agent.title}</span>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-[10px] truncate" style={{ color: S.muted }}>{levelInfo.rank} — {levelInfo.title}</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5">{agent.model}</span>
          </div>
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className="text-[10px]" style={{ color: S.muted }}>{agent.exp} / {nextLevel?.requiredExp ?? '∞'}</span>
          </div>
        </div>
      </div>

      {/* ===== タブバー（横スクロール対応） ===== */}
      <div className="flex border-b border-[#262a38] px-1 overflow-x-auto scrollbar-hide">
        {tabs.map(t => (
