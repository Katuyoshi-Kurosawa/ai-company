...
const CURRENT_SCHEMA_VERSION = 3; // bump when defaultCompany structure changes
...
function syncWithDefaults(cached: Company): Company {
  ...
  let agents = cached.agents
    ...
    .map(a => {
      ...
      return { ...a, parentId: def.parentId, dept: def.dept, title: def.title, personality: def.personality, expertise: def.expertise, stats: def.stats };
    });
